...
from qgis.core import QgsProject, QgsVectorLayer, QgsField, Qgis, QgsStyle, QgsSymbol, QgsRendererCategory, QgsCategorizedSymbolRenderer, QgsMessageLog, QgsVectorDataProvider, QgsExpression, QgsExpressionContext, QgsExpressionContextUtils, QgsVectorFileWriter
...
#Generate a new memory layer with the Interpr field
    ...
    def slot_shp(self):
        ...
        pr = vl.dataProvider()
        pr.addAttributes([QgsField("status", QVariant.String)])
    ...
    def loe_shp(self):
        ...
        pr = vl.dataProvider()
        pr.addAttributes([QgsField("Area", QVariant.Double, 'double', 10,0),
...
#Calculate the Length and the Diameter
    def add_Measure(self,edit):
        ...
        expression1 = QgsExpression('''case when "Interpr" = 'Linear' then $perimeter/100/2/2 when "Interpr" = 'Grave' then $perimeter/100/2/2 when "Interpr" = 'Structure' then $perimeter/100/2/2 end ''')
...
# add the Interpr column with the same code present in the Layer Column
    ...
    def interpr_column(self):
        widget_4 = iface.messageBar().createMessage("Interpretation added", "Well Done")
        vl = iface.activeLayer()
...
# convert the line shp in polygon and add the new layer on the canvas
    def lineTopoly(self):
        widget_4 = iface.messageBar().createMessage("Converted", "Well Done")
